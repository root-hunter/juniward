...
use mozjpeg_sys::*;
use stc::{StcParams, bits_to_bytes, bytes_to_bits, stc_embed, stc_extract};
use std::fs;
...
fn main() {
    ...
    println!(
        "Size: {}x{} blocks, {} total DCT coefficients",
    ...
    println!(
        "  Parameters: h_height={}, num_states={}",
        params.h_height,
        params.num_states()
    );
    ...
    let stego_bits = match stc_embed(&cover_bits, &ac_costs, &message_bits, &params) {
        Ok(bits) => {
            let n_changes: usize = cover_bits
                .iter()
                .zip(bits.iter())
                .filter(|(a, b)| a != b)
                .count();
    ...
    let stego_data =
        unsafe { write_jpeg_dct(&data, &stego_blocks, jpeg.width_blocks, jpeg.height_blocks) };
    ...
    let extracted_bits: Vec<u8> = stego_jpeg
        .blocks
        .iter()
        .enumerate()
        .filter(|(i, _)| i % 64 != 0)
    ...
    let bit_errors: usize = message_bits
        .iter()
    ...
    if bit_errors == 0 {
        ...
        println!(
            "\n✗ Warning: {} bit errors in recovered message",
            bit_errors
        );
    }
}
